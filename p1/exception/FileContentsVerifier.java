...
public class FileContentsVerifier {
	...
	private void appendToFoundInFileString(String linePreﬁx) {
		foundInFileString += linePreﬁx;
	}
	...
	public boolean verifyPlayerString(String lineFromFile, Game game, int armour) {
		...
		if ( !verifyCoords(Integer.parseInt(coords[0]), Integer.parseInt(coords[1]), game)
				|| !verifyLives(Integer.parseInt(words[2]), armour)
				|| !verifyPoints(Integer.parseInt(words[3])) || !verifyBool(words[4]) )
			return false;
		return true;
	} 
	...
	// parseBoolean converts any string diﬀerent from "true" to false.
	...
	// Use a regular expression to verify the string of concatenated preﬁxes found
	public boolean verifyLines() {
		// TO DO: compare foundInFileString with a regular expression 
		return true;
	}
	
	// text explaining allowed concatenated preﬁxes
